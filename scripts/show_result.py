...
def show():
    ...
    df = pd.DataFrame(res)
    df = df.groupby(by='dataset').agg(mean_WOOOD=('w/o OOD', 'mean'), std_WOOOD=('w/o OOD', 'std'),
                                      mean_WOOD=('w/ OOD', 'mean'), std_WOOD=('w/ OOD', 'std')).reset_index()
    ...
    df = pd.DataFrame(res)
    df = df.groupby(by='dataset').agg(mean_WOOOD=('Train', 'mean'), std_WOOOD=('Train', 'std'),
                                      mean_WOOD=('Test', 'mean'), std_WOOD=('Test', 'std')).reset_index()
    df = df.applymap(lambda x: f"{x * 100:.2f}" if isinstance(x, float) else x)
    ...
    print(res1.to_string() + '\n\n' + res2.to_string(), file=open(f"{fdir}/results.txt", 'w'))
